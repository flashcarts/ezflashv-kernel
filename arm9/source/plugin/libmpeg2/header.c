...
static const uint8_t mpeg2_scan_norm_master[64] = {
...
void mpeg2_header_state_init (mpeg2dec_t * mpeg2dec)
{
    ...
    // from idct.c mpeg2_idct_init()
    {
      int i,j;
	  for (i = 0; i < 64; i++) {
	      j = mpeg2_scan_norm_master[i];
	      mpeg2_scan_norm[i] = ((j & 0x36) >> 1) | ((j & 0x09) << 2);
	      j = mpeg2_scan_alt_master[i];
...
//    info->display_picture = info->display_picture_2nd = NULL;
    info->current_picture = NULL;
    info->display_picture = NULL;
...
static void finalize_matrix (mpeg2dec_t * mpeg2dec)
{
	...
	if ((mpeg2dec->copy_matrix & (4 << i)) &&
	    ...
	    decoder->chroma_quantizer[i] = decoder->quantizer_prescale[i+2][0];
	} else if (mpeg2dec->copy_matrix & (5 << i))
	    decoder->chroma_quantizer[i] = decoder->quantizer_prescale[i][0];
    }
}
...
int mpeg2_header_picture (mpeg2dec_t * mpeg2dec)
{
	...
	pmotions->f_motion.f_code[1] = (buffer[3] >> 2) & 1;
	pmotions->f_motion.f_code[0] =
	    (((buffer[3] << 1) | (buffer[4] >> 7)) & 7) - 1;
	pmotions->b_motion.f_code[1] = (buffer[4] >> 6) & 1;
	pmotions->b_motion.f_code[0] = ((buffer[4] >> 3) & 7) - 1;
    }
...
static int picture_coding_ext (mpeg2dec_t * mpeg2dec)
{
    ...
    motions_t *pmotions=decoder->motions;
    pmotions->f_motion.f_code[0] = (buffer[0] & 15) - 1;
    pmotions->f_motion.f_code[1] = (buffer[1] >> 4) - 1;
    pmotions->b_motion.f_code[0] = (buffer[1] & 15) - 1;
    pmotions->b_motion.f_code[1] = (buffer[2] >> 4) - 1;
...
//		       STATE_SLICE : STATE_SLICE_1ST);
	...
	prescale (mpeg2dec, 0);
	if (decoder->chroma_quantizer[0] == decoder->quantizer_prescale[2][0])
	    prescale (mpeg2dec, 2);
	if (mpeg2dec->decoder->coding_type != I_TYPE) {
	    prescale (mpeg2dec, 1);
	    if (decoder->chroma_quantizer[1] == decoder->quantizer_prescale[3][0])
		prescale (mpeg2dec, 3);
	}
    }
