...
#define WAIT(x) { vu32 w; for(w=0;w<x;w++){}; }
...
void SendCommand(u16 command, u32 sectorn)
{
	...
	SDDIR=0x29;
	Hal4ATA_WaitOnBusy();
	WAIT(32); // 2006/11/03 added by Moonlight.
	SDDIR=0x09;
	WAIT(32); // 2006/11/03 added by Moonlight.
}
...
void DMA3(u32 src, u32 dst, u32 cnt)
{
	...
	while(DMA3_CR & DMA_BUSY); // 2006/11/03 added by Moonlight.
}

static u32 DistTemp;

static void PassRespond(u32 num)
{
	u32 dmanum;
	...
	SDCON=0x4;
	WAIT(32); // 2006/11/03 added by Moonlight.
	DMA3(0x8800000,(u32)&DistTemp,0x80400000+dmanum);
	WAIT(32); // 2006/11/03 added by Moonlight.
}

//bool M3SD_write1sector(u32 sectorn,u16 * p)
bool M3SD_write1sector(vu32 sectorn,vu32 p)
{
	u16 crc[4];
	...
	WAIT(1024); // 2006/11/03 added by Moonlight.
