...
#include "gba_nds_fat_chg/lfnunicodedef.h"
bool lfnExists;
...
#ifdef UseFatDump
#include "gba_nds_fat_chg/fatdef.h"
#endif
...
#include "gba_nds_fat_chg/ClustToSect_extern.h"
...
bool FAT_InitFiles (void)
{
	...
	else	// This is an MBR
	{
		...
		_consolePrintOne("MBR boot sector=%d ",bootSector);
	}
...
*/
	...
	_consolePrintOne("RD=%d ",filesysRootDir);
	_consolePrintOne("RE=%d ",bootSec->rootEntries);
	_consolePrintOne("REsize=%d\n",sizeof(DIR_ENT));
	_consolePrintOne("SPC=%d ",bootSec->sectorsPerCluster);
	_consolePrintOne("FD=%d ",filesysData);
	_consolePrintOne("FLC=%d\n",fatLastCluster);

	if (fatLastCluster < 4085)
	{
		_consolePrintOne("Detect FAT12\n",0);
		filesysType = FS_FAT12;	// FAT12 volume - unsupported
	}
	else if (fatLastCluster < 65525)
	{
		_consolePrintOne("Detect FAT16\n",0);
		filesysType = FS_FAT16;	// FAT16 volume
	}
	else
	{
		_consolePrintOne("Detect FAT32\n",0);
	...
	{
		lfnName[i] = '\0';
#include "gba_nds_fat_chg/initfiles1.h"
	}

#ifdef UseFatDump
#include "gba_nds_fat_chg/initfiles2.h"
#endif
...
#include "gba_nds_fat_chg/getlongfilenameunicode.h"
...
#include "gba_nds_fat_chg/fread.h"
#include "gba_nds_fat_chg/fread_fast.h"
...
#include "gba_nds_fat_chg/getattrib.h"
