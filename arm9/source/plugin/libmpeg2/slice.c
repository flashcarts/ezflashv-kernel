 ...
 * $Id: slice.c,v 1.14 2005/02/19 02:32:12 diego Exp $
 */
...
extern CODE_IN_ITCM void (*mpeg2_idct_copy)(int16_t * block, uint8_t * dest, int stride);
extern CODE_IN_ITCM void (*mpeg2_idct_add)(int last, int16_t * block, uint8_t * dest, int stride);
//extern void (* mpeg2_cpu_state_save) (cpu_state_t * state);
//extern void (* mpeg2_cpu_state_restore) (cpu_state_t * state);

static inline void mpeg2_cpu_state_save(cpu_state_t * state)
{
}

static inline void mpeg2_cpu_state_restore(cpu_state_t * state)
{
}
...
#if 0
    	{
    		extern void _consolePrintOne(char *str,u32 v);
    		extern void ShowLogHalt(void);
    		static void (*p_consolePrintOne)(char *str,u32 v)=_consolePrintOne;
    		static void (*pShowLogHalt)(void)=ShowLogHalt;
    		p_consolePrintOne("Can not decode B-FRAME.\n",0);
    		pShowLogHalt();
    	}
    	while(1); // halt.
#endif
...
CODE_IN_ITCM static void get_mpeg1_intra_block (mpeg2_decoder_t * const decoder)
{
    int i;
    int j;
    int val;
...
CODE_IN_ITCM static int get_mpeg1_non_intra_block (mpeg2_decoder_t * const decoder)
{
    int i;
    int j;
    int val;
...
#define bits (decoder->bitstream_bits)
#define bit_ptr (decoder->bitstream_ptr)
#define slice_intra_DCT(decoder,cc,dest,stride) \
{ \
    NEEDBITS (bit_buf, bits, bit_ptr); \
    /* Get the intra DC coefficient and inverse quantize it */ \
    if (cc == 0) \
...
CODE_IN_ITCM static void motion_mp1 (mpeg2_decoder_t * const decoder,
			...
			mpeg2_mc_fct * const * const table)
{
    ...
    MOTION_420 (table, motion->ref1, motion_x, motion_y, 16, 0);
}

#define MOTION_FUNCTIONS2(FORMAT,MOTION,MOTION_FIELD,MOTION_DMV,MOTION_ZERO)   \
									      \
CODE_IN_ITCM static void motion_reuse_##FORMAT (mpeg2_decoder_t * const decoder,	      \
				   motion_t * const motion,		      \
				   mpeg2_mc_fct * const * const table)	      \
{									      \
    int motion_x, motion_y;						      \
    ...
    motion_y = motion->pmv[0][1];					      \
									      \
    MOTION (table, motion->ref1, motion_x, motion_y, 16, 0);		      \
}									      \
									      \
CODE_IN_ITCM static void motion_zero_##FORMAT (mpeg2_decoder_t * const decoder,	      \
				  motion_t * const motion,		      \
				  mpeg2_mc_fct * const * const table)	      \
{									      \
    unsigned int offset;						      \
    ...
    motion->pmv[1][0] = motion->pmv[1][1] = 0;				      \
									      \
    MOTION_ZERO (table, motion->ref1);				      \
}									      \
									      \

MOTION_FUNCTIONS2 (420, MOTION_420, MOTION_FIELD_420, MOTION_DMV_420,
		  MOTION_ZERO_420)

/* like motion_frame, but parsing without actual motion compensation */
CODE_IN_ITCM static void motion_fr_conceal (mpeg2_decoder_t * const decoder)
{
    int tmp;
    motions_t *pmotions=decoder->motions;

    NEEDBITS (bit_buf, bits, bit_ptr);
    tmp = (pmotions->f_motion.pmv[0][0] +
	   get_motion_delta (decoder, pmotions->f_motion.f_code[0]));
    tmp = bound_motion_vector (tmp, pmotions->f_motion.f_code[0]);
    pmotions->f_motion.pmv[1][0] = pmotions->f_motion.pmv[0][0] = tmp;

    NEEDBITS (bit_buf, bits, bit_ptr);
    tmp = (pmotions->f_motion.pmv[0][1] +
	   get_motion_delta (decoder, pmotions->f_motion.f_code[1]));
    tmp = bound_motion_vector (tmp, pmotions->f_motion.f_code[1]);
    pmotions->f_motion.pmv[1][1] = pmotions->f_motion.pmv[0][1] = tmp;

    DUMPBITS (bit_buf, bits, 1); /* remove marker_bit */
}

CODE_IN_ITCM static void motion_fi_conceal (mpeg2_decoder_t * const decoder)
{
    int tmp;
    motions_t *pmotions=decoder->motions;
...
do {								\
	...
	routine (decoder, &(pmotions->f_motion), mpeg2_mc.put);	\
    if ((direction) & MACROBLOCK_MOTION_BACKWARD)		\
	routine (decoder, &(pmotions->b_motion),			\
		      ...
		      uint8_t * forward_fbuf[3], uint8_t * backward_fbuf[3])
{
    ...
    pmotions->f_motion.ref1[0] = forward_fbuf[0] + offset;
    pmotions->f_motion.ref1[1] = forward_fbuf[1] + (offset >> 1);
    pmotions->f_motion.ref1[2] = forward_fbuf[2] + (offset >> 1);

    pmotions->b_motion.ref1[0] = backward_fbuf[0] + offset;
    pmotions->b_motion.ref1[1] = backward_fbuf[1] + (offset >> 1);
    pmotions->b_motion.ref1[2] = backward_fbuf[2] + (offset >> 1);
...
#define bit_ptr (decoder->bitstream_ptr)
    ...
    pmotions->f_motion.pmv[0][0] = pmotions->f_motion.pmv[0][1] = 0;
    pmotions->f_motion.pmv[1][0] = pmotions->f_motion.pmv[1][1] = 0;
    pmotions->b_motion.pmv[0][0] = pmotions->b_motion.pmv[0][1] = 0;
    pmotions->b_motion.pmv[1][0] = pmotions->b_motion.pmv[1][1] = 0;
...
CODE_IN_ITCM void mpeg2_slice (mpeg2_decoder_t * const decoder, const int code,
