...
//    IPC3->strpcmFreq=MP2_GetSampleRate();
  ...
  if(strpcmFormat!=SPF_MP2){
    ...
    strpcmL0=(s16*)safemalloc(strpcmSamples*Multiple*2);//tmp0;
    strpcmL1=(s16*)safemalloc(strpcmSamples*Multiple*2);//tmp1;
    strpcmR0=(s16*)safemalloc(strpcmSamples*Multiple*2);
    strpcmR1=(s16*)safemalloc(strpcmSamples*Multiple*2);
...
//      OutputSamples=Samples;
    ...
    if(OutputSamples==0){
      ...
      if(MP2_GetBitRate()<=128) InterruptHandler_Timer1_ApplyVolume(ldst,rdst,Samples);
...
#include "linkreset_arm7.h"
 void __attribute__ ((long_call)) arm7stage1() {
...
//clear out ARM7 DMA channels and timers
  ...
  POWER_CR = 0 ;  //turn off power to stuffs
...
void __attribute__ ((long_call)) arm7stage1_forGBA() {
