...
static s16 *strpcmMstL,*strpcmMstR;
static u32 strpcmMstFreq,strpcmMstSamples;

static u32 strpcmInterStep;
static u32 strpcmInterPosMod;
static s32 strpcmInterLastL,strpcmInterLastR;
static s32 strpcmInterCurL,strpcmInterCurR;
...
static inline void strpcmPlay()
{
  ...
  strpcmFreq=IPC3->strpcmFreq;
  strpcmSamples=IPC3->strpcmSamples;
  strpcmChannels=IPC3->strpcmChannels;
  
  if(strpcmFormat!=SPF_MP2){
    strpcmMstFreq=strpcmFreq;
    strpcmMstSamples=strpcmSamples;
    strpcmMstL=(s16*)safemalloc(strpcmMstSamples*2);
    strpcmMstR=(s16*)safemalloc(strpcmMstSamples*2);
    
    strpcmFreq=0;
    u64 minmod=0x0000;
    for(u64 seek=0;seek<1024;seek++){
      u64 mod;
      mod=((u64)strpcmMstSamples*(32768+seek)*0x1000)/(u64)strpcmMstFreq;
      mod%=0x4000;
      if(minmod<mod){
        minmod=mod;
        strpcmFreq=32768+(u32)seek;
      }
      mod=((u64)strpcmMstSamples*(32768-seek)*0x1000)/(u64)strpcmMstFreq;
      mod%=0x4000;
      if(minmod<mod){
...
//    if(strpcmFreq!=32667) while(1);
    strpcmSamples=(strpcmMstSamples*strpcmFreq)/strpcmMstFreq;
    
    // mstfreq=48000 freq=32667 mstsamples=576 samples=392.004 step=96,296.813297823491
  ...
  if(strpcmFormat!=SPF_MP2){
    strpcmInterStep=(strpcmMstFreq*0x10000+(strpcmFreq-1))/strpcmFreq;
    strpcmInterPosMod=0;
    strpcmInterLastL=0;
    strpcmInterLastR=0;
    strpcmInterCurL=0;
...
//  SOUND_CR = 0;
  ...
  if(strpcmFormat!=SPF_MP2){
    safefree(strpcmMstL); strpcmMstL=NULL;
    safefree(strpcmMstR); strpcmMstR=NULL;
...
static void InterruptHandler_Timer1_PCM(void)
{
  ...
  u32 MstSamples=strpcmMstSamples;
  ...
  if(IPC3->strpcmWriteRequest!=0){
    ...
    s16 *lmst=strpcmMstL,*rmst=strpcmMstR;
    
    s16 *lsrc=strpcmLBuf;
    s16 *rsrc=strpcmRBuf;
    
    if(strpcmChannels==2){
      MemCopy16DMA3(lsrc,lmst,MstSamples*2);
      MemCopy16DMA3(rsrc,rmst,MstSamples*2);
      }else{
      MemCopy16DMA3(lsrc,lmst,MstSamples*2);
      MemCopy16DMA3(lsrc,rmst,MstSamples*2);
    }
    ...
    lastl=strpcmInterLastL;
    curl=strpcmInterCurL;
    lastr=strpcmInterLastR;
    curr=strpcmInterCurR;
    pos=-1;
    posmod=strpcmInterPosMod;
    step=strpcmInterStep;
    
    for(u32 idx=0;idx<Samples;idx++){
      posmod+=step;
      while(0x10000<=posmod){
        posmod-=0x10000;
        pos++;
        if(pos<MstSamples){
/*
...
*/
          lastl=curl;
          curl=(curl+(s32)lmst[pos])/2;
          lastr=curr;
          curr=(curr+(s32)rmst[pos])/2;
        }
      }
      
      lbuf[idx]=(s16)(((lastl*(0x10000-posmod))+(curl*posmod))/0x10000);
      rbuf[idx]=(s16)(((lastr*(0x10000-posmod))+(curr*posmod))/0x10000);
    }
    
    strpcmInterLastL=lastl;
    strpcmInterLastR=lastr;
    strpcmInterCurL=curl;
    strpcmInterCurR=curr;
    strpcmInterPosMod=posmod;
