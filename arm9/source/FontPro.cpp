...
{ cwl();
  ...
  if(FontProHeight!=FontHeight){ cwl();
...
{ cwl();
  ...
  for(u32 y=0;y<FontProHeight;y++){ cwl();
    for(u32 x=0;x<FontProWidth;x++){ cwl();
...
{ cwl();
  ...
  if(ccode<FontCacheCount){ cwl();
    if(FontCache[ccode].data!=NULL){ cwl();
