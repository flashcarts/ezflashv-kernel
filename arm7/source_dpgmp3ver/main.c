...
#define MP2_SamplePerFrame (1152)

static s16 *strpcmL0=NULL,*strpcmL1=NULL,*strpcmR0=NULL,*strpcmR1=NULL;
static s16 tmp0[MP2_SamplePerFrame*MP2_Multiple]={1,},tmp1[MP2_SamplePerFrame*MP2_Multiple]={1,};
static s16 tmp2[MP2_SamplePerFrame*MP2_Multiple]={1,},tmp3[MP2_SamplePerFrame*MP2_Multiple]={1,};
...
//    IPC3->strpcmFreq=MP2_GetSampleRate();
  ...
  if(strpcmFormat!=SPF_MP2){
    ...
    u32 startadr=0x06000000;
    u32 size=((IPC3->strpcmSamples*Multiple*2)+3)&~4;
    strpcmL0=startadr+(size*0);//(safemalloc(strpcmSamples*Multiple*2);//tmp0;
    strpcmL1=startadr+(size*1);
    strpcmR0=startadr+(size*2);
    strpcmR1=startadr+(size*3);
...
static void InterruptHandler_Timer1_MP2(void)
{
  ...
  if(IPC3->strpcmWriteRequest!=0){
    ...
    if(IPC3->IR_samples<0){
      OutputSamples=Samples;
      }else{
/*
      u32 cnt;
      for(cnt=0;cnt<Samples*MP2_Multiple;cnt++){
        lbuf[cnt]=0;
        rbuf[cnt]=0;
      }
*/
      OutputSamples=UpdateMP2(ldst,rdst);
//      OutputSamples=Samples;
    }
    
    if(OutputSamples==0){
      u32 cnt;
      for(cnt=Samples*MP2_Multiple;cnt!=0;cnt--){
        *lbuf++=0;
        *rbuf++=0;
      }
      }else{
...
/*
    if(strpcmFormat!=SPF_MP2){
      if(VsyncPassed==false) swiWaitForVBlank();
      VsyncPassed=false;
      }else{
      while(VsyncPassed==false){
        if(IPC3->R2YREQ!=R2YREQ_Convert){
          for(u32 w=0;w<0x100;w++);
          }else{
          YUV420toBGR15_CopyFull(IPC3->R2Y_pYBuf,IPC3->R2Y_pUBuf,IPC3->R2Y_pVBuf,IPC3->R2Y_pFrameBuf);
          IPC3->R2YREQ=R2YREQ_Transfer;
        }
      }
      VsyncPassed=false;
    }
