...
#define attriwram 
#undef attrinline
#define attrinline static inline 
...
attrinline word gsm_sub ( word a, word b)
{
	longword diff = (longword)a - (longword)b;
	return saturate(diff);
}

attrinline word gsm_asr (word a, int n)
{
	if (n >= 16) return -(a < 0);
	if (n <= -16) return 0;
	if (n < 0) return a << -n;
...
attrinline word gsm_asl (word a, int n)
{
	if (n >= 16) return 0;
	if (n <= -16) return -(a < 0);
...
attriwram void Gsm_Long_Term_Synthesis_Filtering (
		struct gsm_state* S,
		word	Ncr,
		word	bcr,
		word	* erp,		/* [0..39]		  IN 	*/
...
attriwram void Gsm_RPE_Decoding (
	/*-struct gsm_state *S,-*/
		word xmaxcr,
		word Mcr,
...
attriwram void Gsm_Short_Term_Synthesis_Filter (
		struct gsm_state * S,
